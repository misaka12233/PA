...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, (src2 << 24) >> 24));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = src1 >> ((imm << 7) >> 7));
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->snpc, s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", jalr   , I, R(dest) = s->snpc, s->dnpc = ((src1 + imm) >> 1) << 1);
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->dnpc = (src1 == src2) ? s->pc + imm : s->snpc);
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, s->dnpc = (src1 != src2) ? s->pc + imm : s->snpc);
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , S, R(dest) = src1 + src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , S, R(dest) = src1 - src2);
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , S, R(dest) = src1 << ((src2 << 27) >> 27));
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , S, R(dest) = (src1 < src2) ? 1 : 0);
  INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor    , S, R(dest) = src1 ^ src2);
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , S, R(dest) = src1 | src2);
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , S, R(dest) = src1 & src2);
